...
DEV inline void MutateMetro(CRNG& Rnd, float& v)
{
	...
	const float delta = b * expf(logRatio * Rnd.Get1());
...
DEV inline void MutateScreen(CRNG& Rnd, float* pV, float Min = 0.0f, float Max = 1.0f)
{
	...
	float a = 1.0f / 1024.f, b = 1.0f / 256.f;
	const float logRatio = -logf(b / a);

	const Vec2f rr = Rnd.Get2();

	float delta = (Max - Min) * b * expf(logRatio * rr.x);

	if (rr.y < 0.5f)
	{
		*pV += delta;
		if (*pV >= Max) *pV = Min + (*pV - Max);
	}
	else
	{
		*pV -= delta;
		if (*pV < Min) *pV = Max - (Min - *pV);
	}

	if (*pV < Min || *pV >= Max) *pV = Min;
}
