...
{
	return ((float)(SHRT_MAX) * tex3D(gTexDensity, P.x, P.y, P.z)) / pDevScene->m_SigmaMax;
}
...
{
	return 100.0f * pDevScene->m_TransferFunctions.m_Opacity.F(D);
}
...
// 			return false;
		...
		if (Density(pDevScene, samplePos) / tex3D(gTexExtinction, samplePos.x, samplePos.y, samplePos.z) > RNG.Get1())
		{
			virtualHit = false;
		}
		else
...
DEV inline float SampleDistanceRM(CRay& R, CCudaRNG& RNG, CVolumePoint& VP, CScene* pDevScene, int Component)
{
	...
	while (Sum < S)
	{
		...
		SigmaT	= GetOpacity(pDevScene, Density(pDevScene, VP.m_P)).r;// * (1.0f - GetDiffuse(pDevScene, tex3D(gTexDensity, SamplePos.x, SamplePos.y, SamplePos.z))[Component]);
		Sum		+= SigmaT * Dt;
		MinT	+= Dt;
	}
...
{
	return true;//SampleDistanceRM(R, RNG, VP, pDevScene, 0) > R.m_MaxT;
}
...
//		if (SampleDistanceDdaWoodcock(Re, RNG, VP, pDevScene, CC1))
 		...
 		if (SampleDistanceRM(Re, RNG, VP, pDevScene, CC1) > 0.0f)
		{
			...
			Rl.m_O = pDevScene->m_Lighting.m_Lights[0].m_P;
			Rl.m_D = Normalize(Rl.m_O - VP.m_P);
			Rl.m_MinT = 0.0f;
			Rl.m_MaxT = (Rl.m_O - VP.m_P).Length();
			Li *= Transmittance(pDevScene, Rl.m_O, Rl.m_D, Rl.m_MaxT, 1.0f, RNG);

 			const float D = Density(pDevScene, VP.m_P);
// 
