...
public:
	...
	HOD void ToXYZ(float xyz[3], float* pCIEX, float* pCIEY, float* pCIEZ) const
	{
	...
	{
		float xyz[3];
		ToXYZ(xyz, pCIEX, pCIEY, pCIEZ);
		XYZToRGB(xyz, rgb);
	}

// 	RGBSpectrum ToRGBSpectrum() const;

 	HOD static CColorXyz FromRGB(const float rgb[3], EType type = Reflectance)
	{
		CColorXyz r;
		...
		if (type == Reflectance)
		{
			...
			if (rgb[0] <= rgb[1] && rgb[0] <= rgb[2]) {
				...
				for (int i = 0; i < nSpectralSamples; i++)
					r.c[i] += rgb[0] * RGBRefl2SpectWhite[i];

				if (rgb[1] <= rgb[2])
				{
					for (int i = 0; i < nSpectralSamples; i++)
					{
						r.c[i] += (rgb[1] - rgb[0]) * RGBRefl2SpectCyan[i];
						r.c[i] += (rgb[2] - rgb[1]) * RGBRefl2SpectBlue[i];
					}
				}
				else
				{
					for (int i = 0; i < nSpectralSamples; i++)
					{
						r.c[i] += (rgb[2] - rgb[0]) * RGBRefl2SpectCyan[i];
						r.c[i] += (rgb[1] - rgb[2]) * RGBRefl2SpectGreen[i];
					}
				}
			}
			else if (rgb[1] <= rgb[0] && rgb[1] <= rgb[2])
			{
				...
				for (int i = 0; i < nSpectralSamples; i++)
					r.c[i] += rgb[1] * RGBRefl2SpectWhite[i];

				if (rgb[0] <= rgb[2])
				{
					for (int i = 0; i < nSpectralSamples; i++)
					{
						r.c[i] += (rgb[0] - rgb[1]) * RGBRefl2SpectMagenta[i];
						r.c[i] += (rgb[2] - rgb[0]) * RGBRefl2SpectBlue[i];
					}
				}
				else
				{
					for (int i = 0; i < nSpectralSamples; i++)
					{
						r.c[i] += (rgb[2] - rgb[1]) * RGBRefl2SpectMagenta[i];
						r.c[i] += (rgb[0] - rgb[2]) * RGBRefl2SpectRed[i];
					}
				}
			}
			else
			{
				...
				for (int i = 0; i < nSpectralSamples; i++)
					r.c[i] += rgb[2] * RGBRefl2SpectWhite[i];

				if (rgb[0] <= rgb[1])
				{
					for (int i = 0; i < nSpectralSamples; i++)
					{
						r.c[i] += (rgb[0] - rgb[2]) * RGBRefl2SpectYellow[i];
						r.c[i] += (rgb[1] - rgb[0]) * RGBRefl2SpectGreen[i];
					}
				}
				else
				{
					for (int i = 0; i < nSpectralSamples; i++)
					{
						r.c[i] += (rgb[1] - rgb[2]) * RGBRefl2SpectYellow[i];
						r.c[i] += (rgb[0] - rgb[1]) * RGBRefl2SpectRed[i];
					}
				}
			}
			r *= .94;
		...
		else
		{
			...
			if (rgb[0] <= rgb[1] && rgb[0] <= rgb[2])
			{
				...
				for (int i = 0; i < nSpectralSamples; i++)
					r.c[i] += rgb[0] * RGBIllum2SpectWhite[i];

				if (rgb[1] <= rgb[2])
				{
					for (int i = 0; i < nSpectralSamples; i++)
					{
						r.c[i] += (rgb[1] - rgb[0]) * RGBIllum2SpectCyan[i];
						r.c[i] += (rgb[2] - rgb[1]) * RGBIllum2SpectBlue[i];
					}
				}
				else
				{
					for (int i = 0; i < nSpectralSamples; i++)
					{
						r.c[i] += (rgb[2] - rgb[0]) * RGBIllum2SpectCyan[i];
						r.c[i] += (rgb[1] - rgb[2]) * RGBIllum2SpectGreen[i];
					}
				}
			}
			else if (rgb[1] <= rgb[0] && rgb[1] <= rgb[2])
			{
				...
				for (int i = 0; i < nSpectralSamples; i++)
					r.c[i] += rgb[1] * RGBIllum2SpectWhite[i];

				if (rgb[0] <= rgb[2])
				{
					for (int i = 0; i < nSpectralSamples; i++)
					{
						r.c[i] += (rgb[0] - rgb[1]) * RGBIllum2SpectMagenta[i];
						r.c[i] += (rgb[2] - rgb[0]) * RGBIllum2SpectBlue[i];
					}
				}
				else
				{
					for (int i = 0; i < nSpectralSamples; i++)
					{
						r.c[i] += (rgb[2] - rgb[1]) * RGBIllum2SpectMagenta[i];
						r.c[i] += (rgb[0] - rgb[2]) * RGBIllum2SpectRed[i];
						}
				}
			}
			else
			{
				...
				for (int i = 0; i < nSpectralSamples; i++)
					r.c[i] += rgb[2] * RGBIllum2SpectWhite[i];

				if (rgb[0] <= rgb[1])
				{
					for (int i = 0; i < nSpectralSamples; i++)
					{
						r.c[i] += (rgb[0] - rgb[2]) * RGBIllum2SpectYellow[i];
						r.c[i] += (rgb[1] - rgb[0]) * RGBIllum2SpectGreen[i];
					}
				}
				else
				{
					for (int i = 0; i < nSpectralSamples; i++)
					{
						r.c[i] += (rgb[1] - rgb[2]) * RGBIllum2SpectYellow[i];
						r.c[i] += (rgb[0] - rgb[1]) * RGBIllum2SpectRed[i];
					}
				}
			}
			r *= .86445f;
	...
	HOD static CColorXyz FromRGB(const float& R, const float& G, const float& B, EType type = Reflectance)
	{
		CColorXyz r;
		...
		if (type == Reflectance)
		{
			...
			if (R <= G && R <= B) {
				...
				for (int i = 0; i < nSpectralSamples; i++)
					r.c[i] += R * RGBRefl2SpectWhite[i];

				if (G <= B)
				{
					for (int i = 0; i < nSpectralSamples; i++)
					{
						r.c[i] += (G - R) * RGBRefl2SpectCyan[i];
						r.c[i] += (B - G) * RGBRefl2SpectBlue[i];
					}
				}
				else
				{
					for (int i = 0; i < nSpectralSamples; i++)
					{
						r.c[i] += (B - R) * RGBRefl2SpectCyan[i];
						r.c[i] += (G - B) * RGBRefl2SpectGreen[i];
					}
				}
			}
			else if (G <= R && G <= B)
			{
				...
				for (int i = 0; i < nSpectralSamples; i++)
					r.c[i] += G * RGBRefl2SpectWhite[i];

				if (R <= B)
				{
					for (int i = 0; i < nSpectralSamples; i++)
					{
						r.c[i] += (R - G) * RGBRefl2SpectMagenta[i];
						r.c[i] += (B - R) * RGBRefl2SpectBlue[i];
					}
				}
				else
				{
					for (int i = 0; i < nSpectralSamples; i++)
					{
						r.c[i] += (B - G) * RGBRefl2SpectMagenta[i];
						r.c[i] += (R - B) * RGBRefl2SpectRed[i];
					}
				}
			}
			else
			{
				...
				for (int i = 0; i < nSpectralSamples; i++)
					r.c[i] += B * RGBRefl2SpectWhite[i];

				if (R <= G)
				{
					for (int i = 0; i < nSpectralSamples; i++)
					{
						r.c[i] += (R - B) * RGBRefl2SpectYellow[i];
						r.c[i] += (G - R) * RGBRefl2SpectGreen[i];
					}
				}
				else
				{
					for (int i = 0; i < nSpectralSamples; i++)
					{
						r.c[i] += (G - B) * RGBRefl2SpectYellow[i];
						r.c[i] += (R - G) * RGBRefl2SpectRed[i];
					}
				}
			}
			r *= .94;
		...
		else
		{
			...
			if (R <= G && R <= B)
			{
				...
				for (int i = 0; i < nSpectralSamples; i++)
					r.c[i] += R * RGBIllum2SpectWhite[i];

				if (G <= B)
				{
					for (int i = 0; i < nSpectralSamples; i++)
					{
						r.c[i] += (G - R) * RGBIllum2SpectCyan[i];
						r.c[i] += (B - G) * RGBIllum2SpectBlue[i];
					}
				}
				else
				{
					for (int i = 0; i < nSpectralSamples; i++)
					{
						r.c[i] += (B - R) * RGBIllum2SpectCyan[i];
						r.c[i] += (G - B) * RGBIllum2SpectGreen[i];
					}
				}
			}
			else if (G <= R && G <= B)
			{
				...
				for (int i = 0; i < nSpectralSamples; i++)
					r.c[i] += G * RGBIllum2SpectWhite[i];

				if (R <= B)
				{
					for (int i = 0; i < nSpectralSamples; i++)
					{
						r.c[i] += (R - G) * RGBIllum2SpectMagenta[i];
						r.c[i] += (B - R) * RGBIllum2SpectBlue[i];
					}
				}
				else
				{
					for (int i = 0; i < nSpectralSamples; i++)
					{
						r.c[i] += (B - G) * RGBIllum2SpectMagenta[i];
						r.c[i] += (R - B) * RGBIllum2SpectRed[i];
					}
				}
			}
			else
			{
				...
				for (int i = 0; i < nSpectralSamples; i++)
					r.c[i] += B * RGBIllum2SpectWhite[i];

				if (R <= G)
				{
					for (int i = 0; i < nSpectralSamples; i++)
					{
						r.c[i] += (R - B) * RGBIllum2SpectYellow[i];
						r.c[i] += (G - R) * RGBIllum2SpectGreen[i];
					}
				}
				else
				{
					for (int i = 0; i < nSpectralSamples; i++)
					{
						r.c[i] += (G - B) * RGBIllum2SpectYellow[i];
						r.c[i] += (R - G) * RGBIllum2SpectRed[i];
					}
				}
			}
			r *= .86445f;
